...
lt-expand ../apertium-sv-da.sv.dix | grep -v '<prn><enc>' | grep -e ':<:' -e '\w:\w' | sed 's/:<:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^.<sent>$/g' | tee $TMPDIR/tmp_testvoc1.txt |
        ...
        lt-proc -d ../sv-da.autogen.bin  | sed 's/ \.//g' > $TMPDIR/tmp_testvoc3.txt
...
lt-expand ../apertium-sv-da.da.dix | grep -v '<prn><enc>' | grep -e ':<:' -e '\w:\w' | sed 's/:<:/%/g' | sed 's/:/%/g' | cut -f2 -d'%' |  sed 's/^/^/g' | sed 's/$/$ ^.<sent>$/g' | tee $TMPDIR/tmp_testvoc1.txt |
        apertium-pretransfer|
        apertium-transfer ../apertium-sv-da.da-sv.t1x  ../da-sv.t1x.bin  ../da-sv.autobil.bin | tee $TMPDIR/tmp_testvoc2.txt |
        lt-proc -d ../da-sv.autogen.bin  | sed 's/ \.//g' > $TMPDIR/tmp_testvoc3.txt
