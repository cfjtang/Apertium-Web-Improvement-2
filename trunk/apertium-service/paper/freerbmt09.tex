...
\begin{center}
 ...
 \end{tabular}
\end{center}
\caption{Parameters and return value(s) for the Translate method; if the {\bf srcLang} parameter is omitted, automatic language detection is used to recognize the language used by the text to be translated, and the recognized language is then returned in the {\bf detectedLang} return value, otherwise {\bf detectedLang} is not present.}
\label{tab:api}
\end{table}
