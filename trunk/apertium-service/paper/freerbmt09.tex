...
\begin{center}
 ...
 \end{tabular}
\end{center}
\caption{Parameters and return value(s) for the Translate method; if the {\bf Source Language} parameter is omitted, automatic language detection is applied to recognize the language used in the text to be translated, and the corresponding language code is then returned in the {\bf Detected Source Language} return value; otherwise {\bf Detected Source Language} is not set.}
\label{tab:translate}
\end{table}
...
\begin{itemize}
 ...
 \item Modules dedicated to handling format-related information can make use of regular expression, that often need to compiled into, for example, the corrisponding Finite State Automatas, before they can be effectively used.
\end{itemize}
