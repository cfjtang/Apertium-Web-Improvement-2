...
public class FSTProcessor {
 ...
 public  enum GenerationMode {
   ...
   * Transducers in FSTP
   */
  Map<String, TransExe> transducers;
  ...
  Character readEscaped(InputStreamReader input) throws IOException {
    if (input.read() == -1) {
      streamError();
    }
    ...
    if (input.read() == -1 || (!escaped_chars.contains(val))) {
      streamError();
    }
  ...
  String readFullBlock(InputStreamReader input, Character delim1, Character delim2) throws IOException {
    String result = "";
    result += delim1;
    Character c = delim1;

    while ((c = (char) (input.read())) != -1 && c != delim2) {
  ...
  char readAnalysis(InputStreamReader input) throws IOException {
  ...
  char readPostgeneration(InputStreamReader input) throws IOException {
    if (!input_buffer.isEmpty()) {
      return input_buffer.next();
    }
  ...
  void load(InputStreamReader input) throws IOException {
    // letters
    int len = input.read();
    while (len > 0) {
  ...
  char readSAO(InputStreamReader input) throws IOException {
    if (!input_buffer.isEmpty()) {
      return input_buffer.next();
    }
    ...
    while ((val =  readSAO(input)) != null) {
      ...
      if (current_state.size() != 0) {
        ...
        if (!isAlphabetic(val) && sf.equals("")) {
          ...
          int limit = firstNotAlpha(sf);
          int size = sf.length();
          limit = (limit == Integer.MAX_VALUE ? size :
                  limit);
          input_buffer.back(1 + (size - limit));
          output.write("<d>");
          output.write(sf);
  ...
  void
  setCaseSensitiveMode(boolean value) {
    caseSensitive = value;
  }
