			...
			try:
				for independentSTags in lists[3]:
					if len(set(independentSTags)) != len(independentSTags):
						print(errorsConf["repeatedAttributesPardef"]["message"], entry)
						format = "pardef n=\""+entry+"\">"
						print ("Issue found after the line :",getLineNumber(format))
	...
	for entry in paradigms:
		...
		if len(set(hashTable)) != len(hashTable):
			...
			for position in errorLocation:
				print("Entry Number : ", position[0]+1, "and", position[1]+1, paradigms[entry][position[0]])
		...
		for lists in multiWordEntries[entry]:
			if len(lists) == 4:
				...
				for iterator in range(5):
					try:
						sorted(lists[-(iterator+1)], key=lambda x: (x is None, x))
...
def repeatedTagEntries():
	...
	for entry in multiWordEntries:
		for lists in multiWordEntries[entry]:
			...
			if len(lists) == 8:
				if len(set(lists[-1])) != len(lists[-1]) and len(set(lists[-3])) != len(lists[-1]):
					print(errorsConf["repeatedTagEntries"]["message"], entry)
	...
	for entry in singleWordEntries:
		for lists in singleWordEntries[entry]:
			...
			if(len(lists) == 6):
				if len(set(lists[-1])) != len(lists[-1]) and len(set(lists[-2])) != len(lists[-2]):
					print(errorsConf["repeatedTagEntries"]["message"], entry)
					decideList(lists, -1, -2)

			if(len(lists) == 8):
				if len(set(lists[-1])) != len(lists[-1]) and len(set(lists[-3])) != len(lists[-3]):
					print(errorsConf["repeatedTagEntries"]["message"], entry)
					decideList(lists, -1. -3)
	...
	for entry in singleWordEntries:
		for lists in singleWordEntries[entry]:
			if lists[1] not in validDirection:
				...
				print (errorsConf["transferDirection"]["message"], lists[1], entry)
	...
	for entry in paradigms:
		for lists in paradigms[entry]:
			if lists[0] not in validDirection:
					...
					print (errorsConf["transferDirection"]["message"], lists[0], entry)
...
def blankSpaceDetection():
	...
	for entry in multiWordEntries:
		for lists in multiWordEntries[entry]:
			try:
				for textEntry in filter (lambda textEntry: " " in textEntry, lists[2]):
		...
		for lists in singleWordEntries[entry]:
			...
			for pars in parList:
				...
				if len(temp) != 1:				#Successfully split
					splitPars.append(temp[1])	#Store list of endings of split pars
			...
			if len(replaceableParts) == 0:		#Re-iterate if no part to be split or added
				continue

			if len(lists) == 4:
				text = lists[2]
				for part in replaceableParts:
					if part == '' or text == None:
						continue
					partLen = len(part)
...
def singleWordMainSection(tree):				#Could do with a better name
	...
	for entry in tree.findall(sectionPath):
		...
		try:
			if(singleWordEntries[lemma]):		#This detects if there are repeated entries for one lemma
				pass
		except KeyError:
			singleWordEntries[lemma] = []   	#If entry not already present initiliaze here
		...
		for i in entry.findall(i_path):			#Finds the text in the <i> tag and appends that
			tmpList.append(i.text)

		for ls in entry.findall(l_path):		#Certain Monodix entries are of the type : <e r="LR" lm="bibliographical"><p><l>bibliographical</l><r>bibliographic</r></p><par n="expensive__adj"/></e>
			tmpList.append(ls.text)				#This takes care of adding the text content present in the <l> tag

		for rs in entry.findall(r_path):		#This takes care of adding the text content in the <r> tag
			tmpList.append(rs.text)
			tmpList2 = []

			for s in rs.findall('./s'):			#This is to handle the <j/> tags like : <r>prperss n="p2"/><s n="mf"/><s n="sp"/><j/>have<s n="vbhaver"/><s n="past"/>
		...
		tmpList2 = []							#tmpList2 consists of a list of all the paradigms associated with a given entry
		for pars in entry.findall(par_path):
			tmpList2.append(pars.attrib['n'])
		tmpList.append(tmpList2)

		singleWordEntries[lemma].append(tmpList)#Entries in the dictionary are of the type, "lemma":[<feature list>]

	print("Single word entries loaded")
	return singleWordEntries					#Returns the dictionary of single word entries in the monodix
...
def multiWordMainSection(tree):
	...
	for entry in tree.findall(sectionPath):
		...
		try:
			lemma = entry.attrib['lm']
			if lemma == None or " " not in lemma:	#This prevents any single word entries from creeping in.
				continue
			try:
				if(multiWordEntries[lemma]):		#This detects if there are repeated entries for one lemma
					pass
			except KeyError:
				multiWordEntries[lemma] = []   	#If entry not already present initiliaze here
		except KeyError:
			if lemma == None:
				continue
		...
		for i in entry.findall(i_path):
			tmpList.append(stringify_children(i))	#Here instead of extract just the text from the tags, we pull out the entire content as there might be possible bugs present there
		...
		for rs in entry.findall(r_path):
			...
			gText = None
			for iterator in childList:				#takes care of text enclosed <g>, usually just one entry
				if iterator != None:
					if "<g>" in iterator:
						gText = iterator
		...
		multiWordEntries[lemma].append(tmpList)			##Entries in the dictionary are of the type, "lemma":[<feature list>]
	...
	for paradigm in tree.findall(paradigmPath):
		...
		for es in paradigm.findall(e_path):
			...
			for rs in es.findall(r_path):
				...
				jTagPresent = True if jPos != None else False
				counter = 0
				...
				for ss in rs.findall(s_path):
					counter += 1
					if jTagPresent == True and jPos == counter:
			...
			paradigms[name].append(tmpList)					#Entries in the dictionary are of the type, "paradigm":[<feature list>]
...
def main(arg_list):
	...
	singleWordEntries = singleWordMainSection(tree)		#Could do with a better name
