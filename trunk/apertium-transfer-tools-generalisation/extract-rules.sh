...
if [ "${FLAGS_transfer_tools_1}" == "${FLAGS_TRUE}" ]; then
  ...
  if [ ! -d "$TMPDIR/test-${BEST_TRESHOLD}-count" ]; then
	  ...
	  pushd $TMPDIR/test-${BEST_TRESHOLD}-count
		  ...
		  cat $CORPUS.$SL | gzip > $SL.txt.gz
		  cat $CORPUS.$TL | gzip > $TL.txt.gz
		  make PAR_NO_SHORT_RESTRICTIONS=yes PAR_TH_COUNT=${BEST_TRESHOLD} PAR_ONLY_STANDARD=yes PAR_TESTCORPUS=$TEST_CORPUS PAR_L1=$SL PAR_L2=$TL PAR_CRITERION=count PAR_MAX=$MAX_LENGTH PAR_DISCARD_RULE=yes PAR_RULELEARNINGLIBDIR="$FULLRULELEARNINGDIR" PAR_GIZADIR="$GIZADIR" 2>&1 > $TMPDIR/test-${BEST_TRESHOLD}-count-debug
	  popd
  fi
  ...
  if [ ! -d "$TMPDIR/test-${BEST_TRESHOLD_INV}-count" ]; then
	  ...
	  pushd $TMPDIR/test-${BEST_TRESHOLD_INV}-count
		  ...
		  cat $CORPUS.$SL | gzip > $SL.txt.gz
		  cat $CORPUS.$TL | gzip > $TL.txt.gz
		  make PAR_NO_SHORT_RESTRICTIONS=yes PAR_TH_COUNT=${BEST_TRESHOLD_INV} PAR_ONLY_STANDARD=yes PAR_TESTCORPUS=$TEST_CORPUS PAR_L1=$SL PAR_L2=$TL PAR_CRITERION=count PAR_MAX=$MAX_LENGTH PAR_DISCARD_RULE=yes PAR_RULELEARNINGLIBDIR="$FULLRULELEARNINGDIR" PAR_GIZADIR="$GIZADIR"  2>&1  >$TMPDIR/test-${BEST_TRESHOLD_INV}-count-debug
	  popd
  fi
