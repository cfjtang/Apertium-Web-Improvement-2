...
if __name__=="__main__":
    ...
    parser.add_argument('--extract_all_phrases',action='store_true')
    args = parser.parse_args(sys.argv[1:])

    # variant:
    #    - (nothing): compatible with bildic as in paper
    ...
    if args.extremes_variant != "antiphrases":
         ...
         for line in gzip.open(args.sentences):
            ...
            differenceTLLemmas=len(parallelSentence.tl_lemmas_from_dictionary) - len(parallelSentence.parsed_sl_lexforms)
            if differenceTLLemmas > 0:
                for num in range(differenceTLLemmas):
                    parallelSentence.tl_lemmas_from_dictionary.pop()
            elif differenceTLLemmas < 0:
                for num in range(-differenceTLLemmas):
                    parallelSentence.tl_lemmas_from_dictionary.append(u"dummy")
            ...
            for bilphrase in bilphrases:
                if args.extract_all_phrases:
                    ...
                    if extremesOK:
                        #do rbpe transformation here
                        if args.rbpe:
                             bilphrase.put_restrictions_and_lemmas_for_rbpe()
                        if bilphrase.aligned_words_of_open_categories_match_dictionary_translation(closedCategoriesSet,bilphrase.tl_lemmas_from_dictionary,singleAlignedtoConsiderContrary,openMustBeAlignedWithOpen) or args.variant == "allowincompwithbildic":
                            sl_lemmas,tl_lemmas=bilphrase.extract_lemmas()
                            #if args.rbpe:
                            #    bilphrase.put_restrictions_for_rbpe()
                            #    tllemmasdictorpint=[]
                            #    for i,sllex in enumerate(bilphrase.parsed_sl_lexforms):
                            #        if (sllex.is_unknown()):
                            #            tllemmasdictorpint.append(u"*"+sl_lemmas[i])
                            #        else:
                            #            tllemmasdictorpint.append(sl_lemmas[i])
                            #    print bilphrase.to_string(removeRestrictionsFromLexicalised=False,printDictionaryTranslations=False,useSecondEmptyRepresentation=True)+" | "+u"\t".join(sl_lemmas).encode('utf-8')+" | "+u"\t".join(tl_lemmas).encode('utf-8')+" | "+u"\t".join(tllemmasdictorpint).encode('utf-8')
                            #else:
                            print bilphrase.to_string(removeRestrictionsFromLexicalised=False,printDictionaryTranslations=False,useSecondEmptyRepresentation=True)+" | "+u"\t".join(sl_lemmas).encode('utf-8')+" | "+u"\t".join(tl_lemmas).encode('utf-8')+" | "+u"\t".join(bilphrase.tl_lemmas_from_dictionary).encode('utf-8')
