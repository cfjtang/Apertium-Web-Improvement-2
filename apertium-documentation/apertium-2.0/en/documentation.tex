...
Gema Ramírez Sánchez\\Felipe Sánchez
 Martínez\\ Carme Armentano-Oller\\ Marco A.\ Montava \\ Francis M.\ Tyers\\\\\textbf{EDITOR}:\\Mireia Ginestí
...
\nota{Shouldn't we license this under GPL or another license that is free in Debian terms?

Make sure 1.2 is the right license.

Perhaps we don't want ``or any later version''.

Check the author list. We might have forgotten someone.}
...
español")\nota{Posem un resum de les dades del projecte en un apèndix
  ...
  ací? - Mikel}. It is a shallow-transfer machine translation system,
...
deep-transfer architecture (\emph{Matxin}) that has been developed in
the same project for the pair Spanish-Basque. \emph{Apertium} can
translate at present between the pairs Spanish-Galician,
Spanish--Catalan\footnote{With the name \emph{Catalan} we refer also
  to the Valencian dialectal variant of this language.}
Catalan-Occitan, Catalan-French, and can be used to build translators
between other related language pairs, such as
Danish-Swedish,Czech--Slovak, etc.  \nota{Update the
  language-pair list!}  \nota{I think it is very important to say in this paragraph that the system has been extended}


\nota{The next paragraph needs updating or generalizing:}
...
are \emph{fast} (translating tens of thousands of words per second on
...
(with a fixed equivalent) would produce errors that, in most 
...
independently from the rest of the MT system, for other
...
interoperability, its independence of the character set and the
...
several characters set, which include several characters that look
