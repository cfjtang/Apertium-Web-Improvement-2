...
\nota{falta parlar del lextor, i afegir-ho a tot arreu on es parli dels mòduls del sistema}
  ...
  \item [-]sección \ref{ss:transfer}: \emph{Módulo de transferencia
    ...
    (apartado \ref{functransfer}) y el formato de las reglas de
...
definición de paradigmas en los diccionarios bilingües (véase la página~\pageref{ss:bil}). 

A partir de Apertium 2 existe un nuevo tipo de paradigma, llamado metaparadigma,
que permite definir paradigmas con variaciones según el valor de un atributo
especificado en cada entrada que haga referencia al metaparadigma. En el apartado
\ref{ss:metaparadigmas} se explican sus características y utilización.
...
Al desarrollar los diccionarios para el traductor de occitano surgió
la necesidad de crear una herramienta que permitiera la especificación
de paradigmas para verbos que tenían una misma flexión pero cuya raíz
cambiaba en las diferentes formas flexionadas.  Con el sistema de
especificación de paradigmas existente, era necesario crear un
paradigma diferente para cada verbo, puesto que los paradigmas sólo
permitían especificar regularidades en la flexión de verbos con raíz
invariable. Con los metaparadigmas, se puede especificar la
regularidad de flexión junto con variaciones en la raíz del verbo.

Asimismo, los metaparadigmas permiten también especificar, en un mismo
paradigma, variación de símbolos gramaticales para determinados lemas.
Es decir, varios lemas pueden hacer referencia a un mismo
metaparadigma aunque tengan algún símbolo gramatical diferente. Así
como en el caso del occitano, los metaparadigmas han servido para
tener un mismo paradigma para entradas con variaciones en la raíz,
para el inglés, éstos han servido para tener un mismo paradigma para
entradas con variaciones en los símbolos gramaticales.


A partir de ahí se ha creado el concepto de metadiccionario: un
diccionari que contiene tanto metaparadigmas como paradigmas con el
formato utilizado hasta ahora. El nombre de un diccionario de este
tipo es
\texttt{apertium-}$L_1$\texttt{-}$L_2$\texttt{.}$L_1$\texttt{.metadix}
(por ejemplo, para el diccionario monolingüe inglés de Apertium-en-ca,
\texttt{apertium-en-ca.en.metadix}).
Estos diccionarios son preprocesados cuando se compilan los datos lingüísticos
de modo que tengan un formato apto para el compilador de diccionarios.

\paragraph{Especificación de metaparadigmas}

Los metaparadigmas se definen en la sección
\texttt{<\textbf{pardefs}>} del diccionario monolingüe, que es donde se
definen también los demás paradigmas del diccionario. Un metaparadigma, al igual que un
paradigma, tiene un nombre especificado en el atributo \texttt{n}.
Este nombre tendrá las mismas características que en los demás
paradigmas, con la diferencia de que la parte variable del lema estará
entre corchetes y con las letras en mayúsculas, como se ve en este
ejemplo:
...
Esto define un paradigma para verbo, donde las terminaciones de flexión presentan una parte variable en la raíz. 
La flexión que se especifique en este metaparadigma tiene que ser tal que flexione sólo la parte que hay a la derecha de los 
corchetes, por ejemplo la que se especifica en el paradigma:
...
De este modo, un ejemplo entero de elemento de definición de metaparadigma mediante \texttt{<\textbf{pardef}>} sería:
...
La etiqueta \texttt{<\textbf{prm}/>} es el indicativo que sirve para
situar la parte de texto variable (la terminación variable de la raíz)
dentro de la definición del paradigma.


Así es como se define un metaparadigma. Si ahora queremos que un verbo
lo utilice, en la entrada del verbo (dentro de un
elemento \texttt{<\textbf{e}>}) debe indicarse el metaparadigma correspondiente
y , mediante el atributo \texttt{prm}, especificar por qué letras se debe
sustituir la parte variable cerrada entre corchetes. Por ejemplo:
...
Esta entrada define el verbo \emph{acuélher} y especifica que el paradigma
de flexión que debe seguir es el indicado por el metaparadigma
\texttt{m/é[T]er\_\_vblex} pero sustituyendo \texttt{T} por
\texttt{lh}; es decir, las letras que vienen después de
\emph{acu} serán \emph{élher} en lugar de \emph{éter}.



Como se ha apuntado antes, los metaparadigmas
pueden utilizarse también para entradas que presenten alguna variación
en los símbolos morfológicos. El modo de especificarlos es básicamente el mismo:
la parte variable debe especificarse en la entrada correspondiente,
mediante un atributo \texttt{sa}, y en el paradigma debe colocarse la etiqueta 
\texttt{<\textbf{sa}>} en el lugar en el que debe situarse el símbolo morfológico opcional.
...
en la que \emph{unc} especifica que el nombre es incontable.

En el metaparadigma, la etiqueta \texttt{<\textbf{sa}>} indica el lugar que debe ocupar
la etiqueta, en el caso de que una entrada contenga el atributo \texttt{sa} con un valor, como es
el caso de la entrada para \emph{time}.


Un diccionario que presenta entradas como las descritas se denomina
metadiccionario y tiene que ser procesado para generar un diccionario
que siga la DTD de Apertium 2, puesto que el motor no permite el uso
directo de estos metaparadigmas. A continuación se explica en qué
consiste este procesamiento.




\paragraph{Procesamiento del metadiccionario}


El metadiccionario es un fichero escrito en XML al que, para procesar
los metaparadigmas, se aplican dos hojas de estilo XSLT para obtener
el diccionario con todos los paradigmas derivados de los
metaparadigmas.  Con una primera hoja de estilo,
\texttt{buscaPar.xsl}, se obtienen los verbos que hacen uso de
metaparadigmas y se eliminan posibles repeticiones de metaparadigmas a
expandir. Esta hoja de estilo genera, en combinacio con la hoja
\texttt{principal.xsl}, una segunda hoja de estilo llamada \texttt{gen.xsl}, que procesa
el metadiccionario con la lista de paradigmas a expandir para generar
un diccionario con formato de Apertium 2. Básicamente lo que hace esta hoja de
estilo generada es:

\begin{enumerate}


\item En las entradas de verbos, si el verbo utiliza la flexión de un metaparadigma, se sustituye el metaparadigma por su correspondiente 
paradigma expandido y desparametrizado. Así, la entrada del ejemplo anterior:
...
\end{alltt}
	...
	se desparametrizaría y expandiría a:
...
\item Por otro lado, como de la primera pasada ya se sabe qué
  paradigmas se tienen que crear a partir de los metaparadigmas, éstos
  se crean.  En el caso del ejemplo anterior, a partir del metaparadigma
...
\end{alltt}
	...
	se generaría el paradigma  \texttt{<\textbf{pardef} n="m/élher\_\_vblex"/>} :
...
Después de haber procesado el metadiccionario de esta forma se genera
un diccionario .dix que sigue la DTD de Apertium 2 y que ya se puede
compilar.

 
En el segundo ejemplo que hemos dado, en el que 
la parte variable era la secuencia de símbolos morfológicos del paradigma, a
 partir de la especificación del atributo \texttt{sa} como \emph{unc} y de la aplicación
de las hojas de estilo, se generaría el paradigma:
...
para nombres cuyo análisis morfológico deba ser (en formato del flujo de datos):
...
La utilización de metaparadigmas permite, en este caso, aprovechar un mismo paradigma para entradas con misma flexión pero con análisis
morfológico ligeramente diferente.

Es importante tener en cuenta que, cuando un diccionario 
utilice metaparadigmas y, por lo tanto, tenga un nombre con extensión
\texttt{.metadix}, éste será el fichero en el que deben realizarse todos los
cambios en el diccionario (modificación, adición y borrado de entradas o paradigmas),
puesto que el fichero \texttt{.dix}  se genera automáticamente a partir del primero
cada vez que se compilan los datos lingüísticos y, por lo tanto, cualquier cambio realizado
directamente en este último será sobrescrito al realizarse la compilación.
...
de transferencia estructural más avanzado que el que utilizaba anteriormente,
cuya necesidad aparece con el desarrollo
de traductores para pares de
lenguas menos emparentadas entre sí que los que se habían tratado hasta el
...
un sistema de \textbf{transferencia sintática superficial}, es decir, el sistema
de transferencia utilizado ya para pares de lenguas emparentadas como 
{español}--\emph{catalán} o \emph{español}--\emph{gallego}.
Cuando el sistema se utiliza para pares de lenguas más alejadas y se
desee, por lo tanto, realizar una \textbf{transferencia sintáctica
  avanzada}, se ejecutarán los tres módulos de transferencia.

Los dos sistemas de transferencia se diferencian por el número de
pasadas que se realizan sobre el texto de entrada.  El sistema de
transferencia superficial realiza las transformaciones estructurales
en una única pasada de las reglas, las cuales detectan secuencias o
\emph{patrones} de formas léxicas y realizan sobre ellas las
comprobacions y cambios pertinentes. Por otro lado, el sistema de
transferencia avanzado funciona con una nueva arquitectura de
transferencia que permite detectar \emph{patrones de patrones} de
formas léxicas mediante tres pasadas, que realizan sus tres módulos.

A continuación se describen sus características.  En el apartado
\ref{functransfer} se describe el sistema de transferencia superficial
y en el apartado \ref{apertium2}, el sistema de transferencia avanzada.
La descripción del sistema de transferencia superficial es aplicable también 
al funcionamiento del primero módulo del sistema de transferencia avanzada, con las
diferencias que se exponen en el apartado correspondiente. 
En el apartado \ref{formatotransfer} se describe el formato utilizado
para crear reglas en ambos sistemas. En el apartado \ref{noutransfer} se describe en detalle el funcionamiento
de los tres módulos del sistema de transferencia avanzada y finalmente, en el apartado \ref{ss:preproceso_transfer}
se explica el preprocesamiento de los módulos para su ejecución.
...
\subsection{Transferencia sintáctica avanzada}
\label{apertium2}
...
Como contraste, la
nueva arquitectura de transferencia avanzada  se
...
La arquitectura de transferencia avanzada
...
\begin{itemize} 
\item \texttt{chunker}: identifica los segmentos, realiza la
  traducción palabra por palabra, así como ciertas operaciones de
  reordenamiento y propagación de información morfosintáctica dentro
  del segmento (por ejemplo, para establecer la concordancia). Además,
  crea los segmentos para que sean tratados por el módulo siguiente.
  El \texttt{chunker} tiene la opción de funcionar como único módulo
  en un sistema de transferencia sintáctica superficial.  Ello se
  controla mediante un atributo del elemento \texttt{<transfer>}.
...
módulos del sistema de transferencia (sección \ref{sec:format}) y el tratamiento de 
...
El siguiente apartado, \ref{formatotransfer},  está dedicado a la descripción del 
formato de las reglas de
transferencia, que es común para los tres módulos y para los dos modos de transferencia, con algunas diferencias.
Finalmente, tras esta descripción, en \ref{noutransfer} se describen en más detalle los tres módulos de los que
se compone un sistema de transferencia sintáctiva avanzada.
...
(\textit{Sólo en el módulo chunker})

Es el elemento raíz  del
módulo \texttt{chunker} y contiene todos los demás
...
\item \texttt{lu}: indica que funcionará en modo superficial, es decir, como único módulo de
...
\item \texttt{chunk}: indica que funcionará en modo avanzado y, por lo tanto, cuando una palabra no es
reconocida por ninguna regla,  se creará de oficio un segmento que encapsule
esta palabra para que pueda ser tratada por los siguientes módulos de transferencia 
de un sisema de transferencia
sintáctica avanzada.
...
\paragraph{Uso en el módulo chunker (transferencia superficial y transferencia avanzada)}
...
\item \texttt{link-to} \textit{(sólo en \texttt{chunker} en modo
    avanzado)} sustituye el valor que se derivaría de la consulta del
  resto de atributos del clip por el valor especificado en este
  atributo, que ha de ser un número natural ($>0$). Este número indica
  con qué etiqueta \texttt{<\textbf{tag}>} del
  \texttt{<\textbf{chunk}>} se enlaza el contenido del clip, siendo el
  número el orden que ocupa esta etiqueta dentro de
  \texttt{<\textbf{tags}>}. El resto de atributos del clip quedan
  únicamente como información, pues son sobrescritos por el valor de
  la etiqueta enlazada. Puede verse un ejemplo de su uso en la figura
...
\item \textbf{Uso en \texttt{chunker} en modo superficial y en \texttt{postchunk}}
...
\item \textbf{Uso en \texttt{chunker} en modo avanzado}
...
se utiliza en el módulo \texttt{chunker} en modo avanzado y en el módulo \texttt{interchunk}.
En el módulo \texttt{postchunk} no se usa porque no es necesario crear
ningú chunk a la salida. En el módulo \texttt{chunker} en modo superficial
tampoco seusa porque a la salida no se envían chunks sino unidades léxicas individuales
y blancos.

\begin{enumerate}

\item \textbf{Uso en el módulo chunker en modo avanzado}
...
\textit{Sólo en chunker en modo avanzado}. 
...
\subsection{Especificación de los tres módulos del sistema de transferencia avanzada} 
\label{noutransfer}
...
primero módulo, llamado \texttt{chunker}, que se comunica directamente con el
módulo de generación.


\subsubsection{Programa \texttt{chunker}}
\label{ss:chunker}
...
avanzada. Un atributo del elemento \texttt{<transfer>} controla
...
\item Salida:
\begin{itemize}
\item[-] en modo avanzado (en un sistema de transferencia avanzada): segmentos (chunks),
  para que el módulo siguiente los detecte y realice operaciones con ellos
\item[-]  en modo superficial (en un sistema de transferencia superficial): formas léxicas que constituyen
...
precompilado de detección de patrones calculado a partir del primero. El nombre del
...
El sistema de detección de reglas de este módulo será el 
que se ha explicado en \ref{functransfer}
pues es el mismo en modo de transferencia avanzada y en modo de 
transferencia superficial. El programa
...
El funcionamiento es básicamente similar en el modo
superificial y en el modo avanzado, con estas diferencias:

\begin{itemize}
\item Para que el módulo funcione como primer módulo
de un sistema de transferencia sintáctica avanzada, en el atributo
opcional \texttt{default}  del elemento raíz \texttt{<transfer>}
debe especificarse el valor \texttt{chunk}. El valor por defecto
es \texttt{lu}, que implica que el \texttt{chunker} funcione
en modo de transferencia sintáctica superficial (como único módulo).

\item Generación de segmentos (chunks) a la salida: la etiqueta \texttt{<chunk>} es una etiqueta de un nivel más alto
...
\begin{itemize}
\item \texttt{chunk}: cuando el módulo funcione en modo avanzado
...
\item \texttt{lu} (valor implícito): cuando el módulo funcione como
  único módulo de transferencia en modo superficial (transferencia
...
\texttt{chunker} i en el \texttt{in\-ter\-chunk}. 
...
\chapter{Formularios web de inserción de datos}



Este capítulo presenta el sistema de mantenimiento de diccionarios
de Apertium 2.  Está estructurado en dos secciones.  En la sección
\ref{ss:formadmin} se da la información necesaria para instalar la herramienta
y realizar adaptaciones.  En la sección \ref{ss:formus} se expone el
modo de uso de la aplicación web para llevar a cabo la mejora de los datos
lingüísticos.


\section{Introducción}

La tarea de introducir nuevos términos a los diccionarios de las
diferentes lenguas del traductor es lenta si se hace editando
manualmente los diccionarios en XML; para ello se han creado estos
formularios, que facilitan considerablemente la tarea de inserción de
palabras y además permiten hacerlo de forma remota desde cualquier
ordenador que tenga acceso a Internet.

Así se plantean como unos formularios escritos en \texttt{php}
que son utilitzables con cualquier navegador de Internet, ya sea de forma
local en el mismo ordenador donde se almacenan los diccionarios, ya sea
remotamente.

\section{Instalación y administración}
\label{ss:formadmin}

\subsection{Instalación de la herramienta}

La instalación debe realizarse en un máquina Unix que tenga
instalado un servidor web Apache con \texttt{php}. Así, primero
debe hacerse la instalación del servidor \texttt{php} en caso de que
no estuviera hecha, y a continuación puede ya instalarse la herramienta
de formularios.


Para ello, debe descargarse el paquete \textit{`apertium-lexical-webform-0.9'} de la página
web de Apertium  en Sourceforge (\url{http://apertium.sourceforge.net/}) 
y descomprimirlo dentro del directorio donde vaya a dejarse la herramienta.
...
Hay que tener en cuenta que Apache sólo sirve las páginas que
están dentro del directorio raíz con el que lo hemos configurado. Por lo tanto, el
directorio donde dejemos los formularios tiene que ser un subdirectorio que
esté dentro de la raíz del servidor Apache.

A continuación debe editarse el fichero de configuración que está en
\textit{private/config.php} y dar valores adecuados a las variables
de configuración:

\begin{itemize}
\item \texttt{\$anmor}: ruta entera donde se encuentra el analizador
  morfológico \texttt{lt-proc}.
\item \texttt{\$dicos\_path}: ruta donde se encuentran los diccionarios finales
  y los binarios compilados de cada diccionario. Este directorio tiene que
  contener un subdirectorio para cada diccionario con el que puede
  trabajar el formulario. El nombre del subdirectorio tiene que seguir la siguiente
  sintaxis: \texttt{paradigmes-ll-rr} , donde \textit{ll} y \textit{rr}
  son las iniciales del par de lenguas del traductor en
  concreto. En cada directorio se dejan los diccionarios finales con los que
  trabaja el traductor y los correspondientes binarios compilados.
  Estos directorios se pueden sustituir por enlaces simbólicos en caso de que
  se encuentren en diferentes lugares.
\item \texttt{\$usuaris\_professionals}: un listado de los usuarios
  profesionales del sistema que tienen permiso para inserir en los
  diccionarios del formulario y validar entradas (equivalencias) pendientes de confirmación.
  
\item \texttt{\$mail}: Dirección electrónica del administrador
  encargado de los formularios. Cuando alguien solicite darse de alta como
  usuario se enviará un correo electrónico a esta dirección.
\end{itemize}

Una vez configurados los parámetros de este fichero, el servidor
de formularios ya se encuentra en funcionamiento.


\subsection{Estructura de directorios}

Todos los ficheros necesarios para el correcto funcionamiento se estructuran
de la siguiente forma:

\begin{itemize}
\item \texttt{/index.php:} presenta el formulario inicial de
  inserción.  Tiene un apartado para cada par de lenguas donde se
  introduce el lema en lengua origen y en lengua meta y se elige
  el tipo de categoría gramatical apropiado. Al pulsar el botón
  \textit{'Go on'} se pasa a la página siguiente, donde deben
  seleccionarse los paradigmas de flexión correspondientes a cada uno
  de los lemas en lengua origen y lengua meta.
\item \texttt{/dics:} directorio con los diccionarios de las entradas
  que van inseriéndose desde los formularios. Están los ficheros con las
  entradas de los usuarios no profesionales (pendientes de ser validades) y los
  diccionarios con las entradas en formato \texttt{XML} de los usuarios
  profesionales.
\item \texttt{/private:} aquí está la mayor parte de los módulos
  utilizados en los formularios. También contiene los directorios con
  la definición de paradigmas para todas las lenguas con las que podemos
  trabajar; estos directorios tienen la forma
  \texttt{paradigmes-ll-rr} siendo \textit{ll} y \textit{rr} las
  iniciales del par de lenguas del traductor en concreto. El orden
  escogido para las dos lenguas, primero \textit{ll} y después
  \textit{rr}, depende del orden en que se hayan construido las entradas
  para el diccionario bilingüe. También están en este directorio los
  ficheros encargados de hacer todo el procesamiento de las palabras a inserir.
  Estos son:
\begin{itemize}
\item \texttt{resultado.php: } Este \textit{php} es llamado cuando se
  insieren dos palabras de cualquier par de lenguas desde el módulo
  \textit{index.php}. Básicamente lo que hace es establecer el par de
  lenguas con que se trabajará (\textit{\$LR} y \textit{\$RL}) y la
  categoría gramatical de la palabra que se inserirá
  (\textit{\$tipus}). Se incluye en el módulo \textit{selec.php} que
  será el siguiente que se llama en el proceso de inserción. En el
  supuesto de que el \textit{tipus} (\textit{tipo}) de la palabra a
  inserir fuera una unidad multipalabra (\textit{Multi Word Verb})
  entonces es el módulo \textit{multip.php} el que se incluye y se
  llama en vez de \textit{selec.php}. Los elementos \textit{Multi Word
    Verb} o unidades multipalabra están formados por un verbo que
  puede flexionar seguido de una cola invariable de una o más palabras
  (véase una descripción más detallada en la sección
  \ref{ss:multipalabras}.
\item \texttt{selecc.php: } Es el módulo encargado de proporcionar la selección
  del paradigma escogido para cada una de las dos palabras,
  la palabra en lengua origen y la palabra en lengua meta. Proporciona una
  lista de paradigmas para escoger que dependen del tipo gramatical
  de la entrada que estamos creando. Cuando se selecciona un nuevo paradigma
  para uno de los dos lemas, proporciona unos ejemplos de la flexión del
  lema a inserir según el paradigma que hemos escogido. Si aceptamos los
  paradigmas escogidos, el módulo llama a \textit{insertarPro.php} o \textit{insertar.php}
  dependiendo de si el usuario que está haciendo la nueva
  inserción está considerado como profesional o no profesional
  respectivamente.
\item \texttt{multip.php: } Tiene la misma función que el módulo
  \textit{selecc.php} pero para las unidades multipalabra. Utiliza
  las mismas variables y realiza las mismas operaciones, pero cuando
  muestra los ejemplos lo hace flexionando el verbo y añadiendo todas las
  palabras que forman la cola. La manera de funcionar del módulo es
  análoga a la del módulo \textit{selecc.php} y su descripción
  detallada la encontraremos en el apartado \ref{ss:fitxersphp}.
\item \texttt{valida.php: } Este módulo se llama cuando un usuario
  profesional decide validar palabras que están en la cola
  de entradas pendientes de validación. Lee el fichero con la cola de palabras
  por validar y lo recorre de una en una, coge los datos de la
  entrada correspondiente (\textit{LRlem, RLlem, paradigmaLR,
    paradigmaRL, LR, RL}, etc...) y llama a \textit{selecc.php} para
  continuar con el proceso de inserción de esa entrada particular.
\item \texttt{insertarPro.php: } Este módulo se llama cuando ya hemos
  seleccionado el paradigma adecuado para la palabra en lengua origen
  y en lengua meta (operación realizada en \textit{selecc.php}) y queremos
  visualizar como quedará la entrada en el formato \texttt{XML}
  de los tres diccionarios, monolingüe origen, bilingüe y monolingüe
  meta. Desde esta pantalla se podrá modificar directamente el código
  y finalmente aceptar la nueva entrada o cancelar la operación.
\item \texttt{ins\_multip.php: } Tiene la misma función que
  \textit{insertarPro.php} pero está pensado para las unidades
  multipalabra, de forma que tiene un tratamiento especial con la entrada
  para que el código \texttt{XML} que se insiere en los diccionarios sea
  correcto.
\item \texttt{insertar.php: } Es el módulo equivalente a \textit{insertarPro.php}
  pero cuando la operación está realizándola un
  usuario no profesional. Las acciones que se realizan en este caso son
  mucho más sencillas puesto que la única cosa que se hace es inserir en
  el fichero de términos pendientes de validar los lemas y los paradigmas
  escogidos por el usuario no profesional, y quedan allí hasta que un
  usuario profesional los valida.
\item \texttt{verSemi.php: } Este módulo tiene la función de mostrar el
  fichero con las entradas inseridas por usuarios no profesionales y que
  están pendientes de ser validadas. Puede servir como ayuda para los
  usuarios profesionales que antes de empezar a validar palabras quieran
  comprobar cuáles están en la cola para validar. Para llamarlo
  aparece un enlace en el formulario generado por \textit{selec.php} desde donde
  se puede llamar esta funcionalidad.
\item \texttt{paradigmas.xsl:} Hoja de estilos que se utiliza para
  generar los ficheros de paradigmas con que trabajan los módulos del
  formulario. Se utiliza con la especificación de los paradigmas de cada
  lengua escrita en formato \texttt{XML}. Este punto se tratará más
  extensamente en el apartado \textit{Ficheros de Paradigmas}.
\item \texttt{creaparadigma.awk:} Fichero \texttt{awk} que también
  se utiliza para generar los ficheros de paradigmas de trabajo.
\item \texttt{gen\_paradig.sh:} Script que se puede usar si queremos
  que automáticamente se generan los ficheros de paradigmas de todos los
  pares de lenguas que tenemos instalados.
\end{itemize}
\end{itemize}

La especificación detallada de las tareas de cada módulo se encuentra en las
secciones correspondientes.

\subsection{Ficheros php}

\subsubsection{resultado.php}

Dependiendo del valor de la variable \texttt{\$nomtrad} actualizada por
\textit{index.php} el módulo asigna los valor adecuados a \texttt{\$LR}
y \texttt{\$RL} (la lengua origen y la lengua meta
respectivamente). Seguidamente, dependiendo de la categoría gramatical de la
palabra que se inserirá, asignamos el valor adecuado a la variable
\$tipo y llamamos a \textit{selec.php} o \textit{multip.php} dependiendo
de si se trata de una palabra simple o de una unidad multipalabra.
\nota{MG: ``asignamos'' i ``llamamos'' no seria més aviat ``se asigna'' y ``se llama''?}

\subsubsection{selecc.php}
\label{ss:fitxersphp}

Este módulo tiene la función de elegir el paradigma correspondiente a las
palabras que queremos introducir. Se elige el paradigma de la palabra en
lengua origen y en lengua meta.

En función de la categoría gramatical de la palabra que estamos
introduciendo, se asignan unos determinados valores a unas variables que
darrerament se utilizarán \nota{MG: vol dir ``que se utilizarán más adelante''?}, estas son
\begin{itemize}
\item \texttt{cadFich:} categoría gramatical del lema.
\item \texttt{show:} cadena que se muestra en el  formulario indicando la categoría
  gramatical de la palabra que está inseriéndose.
\item \texttt{tag:} cadena con la etiqueta \texttt{XML} de salida del
  analizador morfológico para esta categoría gramatical.
\item \texttt{tagout:} cadena con el código \texttt{XML} que indica la categoría
  gramatical de la palabra. Esta cadena se utilizará cuando se
  componga la entrada final \texttt{XML} que se inserirá en el diccionario.
\item \texttt{nota:} cadena con posibles comentarios a inserir en
  el código \texttt{XML} de la entrada.
\end{itemize}
Los formularios trabajan con 4 tipos de diccionarios:
\begin{itemize}
\item \textit{Semiprofesionales}: Estos contienen las palabras
  inseridas desde el formulario por usuarios no profesionales y que están
  pendientes de validación. Acaban con la extensión "\textit{semi.dic}"
\item \textit{Diccionarios del formulario}: Contienen las palabras que
  han sido inseridas desde el formulario por usuarios profesionales y también
  las que han sido validades desde los diccionarios
  semiprofesionales. Acaban con la extensión "\textit{webform}".
\item \textit{Finales}: Son los ficheros con todas las entradas escritas
  en código \texttt{XML} . Estos ficheros son los que utiliza
  definitivamente el traductor después de recibir un tratamiento adecuado
  de compilación. Acaban con la extensión "\textit{dix}".
\item \textit{Finales compilados}: Después de haber compilado los
  diccionarios finales ya pueden ser utilizados por los binarios del
  traductor. Acaban con la extensión "\textit{bin}"
\end{itemize}

Estos diccionarios son utilizados por los formularios y hay variables
que contienen las rutas donde se pueden encontrar. También se dan valores a las
variables que mantienen las rutas donde se tienen que buscar los ficheros
auxiliares y de configuración:
\begin{itemize}
\item \texttt{path:} ruta para los diccionarios temporales.
\item \texttt{fich\_LR:} diccionario de la lengua origen de las
  palabras inseridas mediante el formulario y que todavía no están en el
  diccionario final ni en el diccionario compilado.
\item \texttt{fich\_RL:} diccionario de la lengua meta de las palabras
  inseridas mediante el formulario y que todavía no están en el diccionario
  final ni en el diccionario compilado.
\item \texttt{fich\_LRRL:} diccionario bilingüe de las palabras
  inseridas mediante el formulario y que todavía no están en el diccionario
  final ni en el diccionario compilado.
\item \texttt{fich-semi:} entradas inseridas mediante el formulario
  por usuarios no profesionales y que están pendientes de validación.
\item \texttt{path\_paradigmasLR:} ruta de los ficheros con los
  paradigmas de flexión para la lengua origen.
\item \texttt{path\_paradigmasRL:} ruta de los ficheros con los
  paradigmas de flexión para la lengua meta.
\item \texttt{anmor:} ruta del analizador morfológico.
\item \texttt{aut\_LRRL:} ruta del binario morfológico de lengua origen
  a lengua meta.\nota{MG: no serà ``binario bilingüe''?}
\item \texttt{aut\_RLLR:} ruta del binario morfológico de lengua meta a lengua
  origen.\nota{MG: ídem}
\end{itemize}

A continuación se insiere el código html de lo que hay que hacer según la
acción seleccionada. Las acciones que se producen por orden
secuencial son las siguientes:

\begin{itemize}
\item Comprueba que el lema en lengua origen a inserir no esté ya
  en los diccionarios de palabras inseridas por el formulario. Si se ha
  llamado a \texttt{selecc.php} desde la pantalla de validación de palabras
  (\texttt{valida.php}), entonces comprueba que el lema no 
  esté ya en el fichero de palabras inseridas por usuarios no
  profesionales. También lo comprueba en el diccionario completo final.
\item Hace la misma comprobación correspondiente para la lengua meta.
\item Seguidamente se escribe código para seleccionar restricciones del
  sentido de la traducción.
\item Se definen una serie de funciones que se utilizarán en la generación
  de ejemplos para los lemas cuando elegimos el correspondiente
  paradigma. Estas son:
  \begin{itemize}
  ...
  \end{itemize}
  Estas funciones se describen más adelante en el apartado
  correspondiente a \textit{insertarPro.php}.
\item Se abre el fichero de paradigmas para mostrar una ventana de selección
  desplegable con los paradigmas que podemos elegir para el lema
  en lengua origen que estamos tratando. Para hacer esto se tienen que
  comprobar secuencialmente los paradigmas correspondientes a la categoría
  gramatical del lema y comprobar que el paradigma se puede aplicar al
  lema correspondiente.
\item A continuación se hace lo mismo pero para los paradigmas del lema en
  lengua meta.
\item Con los lemas y con los paradigmas correspondientes seleccionados,
  se tienen que generar los ejemplos de cómo quedaría la flexión de esos
  lemas según los paradigmas escogidos. Para conseguir esto
  necesitamos, por un lado, la raíz del lema (\texttt{raiz\_LR y raiz\_RL})
    y, por otro, las terminacions de ejemplo para el paradigma
  escogido (\texttt{paradigma\_LR y paradigma\_RL}), terminaciones que
  se obtienen del fichero de paradigmas. Finalmente se compone una cadena con
  los ejemplos generados (\texttt{ejemplos\_LR y ejemplos\_RL}) y se
  muestran.
\item Si estamos en esta pantalla porque venimos de validar palabras
  (\texttt{va\-li\-da=1}) entonces se añade al formulario un botón para
  eliminar la entrada actual en caso de que no queramos validarla e
  inserirla.
\item Si el usuario que ha entrado en esta pantalla es un usuario
  reconocido como profesional, entonces se añade al formulario un botón
  para que ese usuario pueda elegir la opción de validar palabras
  inseridas por usuarios no profesionales
\item Finalmente tenemos el tratamiento de la acción correspondiente activada
  al pulsar uno de los botones de confirmación que se muestran en la
  parte inferior del formulario. Si la acción es \textit{"Delete"} , lo cual
  sólo puede ocurrir cuando se están validando entradas, se elimina la entrada
  correspondiente del fichero con las entradas de usuarios no profesionales.
  Si la acción es la de confirmar (botón \textit{"Go on"}), se llama
  al módulo \texttt{insertarPro.php} o \texttt{insertar.php} dependiendo
  de si el usuario es profesional o no profesional respectivamente.
  Estos módulos se encargan de hacer la inserción en los diccionarios.
\end{itemize}
Cuando se ha inserido la palabra, se vuelve a mostrar la página de \texttt{va\-li\-dar.php}
o de \texttt{selecc.php} según si habíamos
entrado desde un procedimiento de validación (y entonces
\textit{valida=1}) o desde una inserción normal.

\subsubsection{multip.php}
El código y el comportamiento de este módulo es el mismo que el de \textit{selecc.php}.
La única diferencia es que este está pensado para
tratar las unidades multipalabra, mientras que \textit{selec.php}
trata el resto de unidades. Así, la diferencia más evidente es que
tenemos las variables \texttt{\$LRcua} y \texttt{\$RLcua} que contienen
la cola invariable que va después de la parte variable de la
multipalabra. Cuando se muestran los ejemplos, aparte de mostrar la parte
variable flexionada según el paradigma escogido, también se muestra
...
diccionarios, se llama al módulo \textit{ins\_multip.php} en vez de \textit{insertarPro.php}..


\subsubsection{valida.php}
Este módulo es llamado cuando un usuario profesional pulsa el botón
"\textit{validate pairs}". El control pasa a este módulo que lee
el diccionario de entradas pendientes de ser validadas (\$fichSemi) del
par de lenguas correspondiente. Entonces se entra en un bucle que
recorre este fichero y va leyendo las entradas una a una. Con
la información de la entrada particular da valor a una serie de variables
que serán utilizadas en los módulos que harán el tratamiento posterior,
como por ejemplo:
\begin{center}
% use packages: array
\begin{tabular}{ll}
...
Una vez establecidos los valores adecuados para estas variables se
cede el control a \textit{selec.php} que trata la entrada como si
de una inserción de usuario profesional se tratara. Después de inserir
las entradas en los diccionarios mediante \textit{insertarPro.php}, el
flujo vuelve a \textit{valida.php} que continúa con la siguiente entrada
por validar.

\subsubsection{insertarPro.php} 
Una vez introducidos los lemas y seleccionados los paradigmas
correspondientes en \textit{selec.php}, este módulo es el encargado de generar
las correspondientes entradas en código \texttt{XML} e inserirlas
en los diccionarios monolingües y en el bilingüe.

Realiza muchas operaciones parecidas a las realizadas en
\textit{selec.php}, como por ejemplo generar los ejemplos de la palabra
flexionada. Así primeramente da valor a \texttt{cadFich, show, tag,
  tagout, nota} dependiendo de la categoría gramatical (\texttt{\$tipus})
de la palabra a inserir.  Asigna rutas a las variables de ubicación de ficheros
y define unas funciones necesarias de igual manera a como
ocurría en \textit{selec.php}.
\begin{itemize}
\item \texttt{esVocalFuerte}: Devuelve \textit{true} si la vocal es
  fuerte, es decir \textit{a, e, o}.
\item \texttt{esVocalDebil}: Devuelve \textit{true} si la vocal es
  débil, es decir \textit{a, e, o}.\nota{MG: vol dir ``i, u''?}
\item \texttt{esVocal}: Devuelve \textit{true} si el carácter que le pasamos
  como argumento es una vocal.
\item \texttt{diptongo}: Devuelve \textit{true} si las dos letras que
  se le pasan como argumentos forman diptongo. Esto ocurrirá siempre que
  las dos vocales no sean fuertes.
\item \texttt{acentuar}: Se le pasa una cadena de texto y lo acentúa
  siguiendo las reglas del castellano en función del parámetro
  \textit{\$siguienteletra}.
\item \texttt{esMayuscula}: Devuelve \textit{true} si el carácter está
  en mayúscula.
\item \texttt{TieneAcento}: Devuelve \textit{true} si la cadena tiene
  acento.
\item \texttt{acentua}: Acentúa la última vocal accentuable de una
  palabra con un acento cerrado o abierto dependiendo del sentido indicado en
  el parámetro \$sentit.
\item \texttt{PonQuitaAcento}: Pone o quita el acento de la primera
  cadena pasada como argumento en función de si la segunda cadena
  argumento tiene o no tiene acento.
\item \texttt{PosicioVocalTall}: Devuelve la posición dentro del lema
  (\$lema) donde se encuentra la vocal (\$vocal) que separa la raíz de la
  desinencia. Se busca la vocal del final hacia el principio y se devuelve la primera
  ocurrencia de \$vocal.
\end{itemize}

Ahora se realizan las mismas operaciones que en \textit{selec.php}. Primero,
comprueba que la entrada no esté ya en los diccionarios y genera los
ejemplos de la palabra flexionada con el paradigma previamente
escogido.  A continuación se construye la cadena con el código
\texttt{XML} que va a inserirse en el diccionario monolingüe de lengua
origen. Con la información que tenemos de los lemas proveniente de \textit{selec.php},
se genera una cadena de texto (\texttt{\$cad\_LR})
que contiene el código \texttt{XML} para el diccionario monolingüe. Esta
cadena se muestra en una ventana de texto que puede ser modificada
manualment. El mismo proceso se repite para generar la cadena
correspondiente al diccionario monolingüe de lengua meta
(\texttt{\$cad\_RL}) y para el diccionario bilingüe
(\texttt{\$cad\_bil}).  Luego, se concatena a estas variables
de cadena los comentarios y el nombre del usuario que insiere la entrada, si
procede.  Finalmente se acaba de componer la pantalla de formulario, con
botones de aceptar, eliminar y volver atrás.  El código que hace el
tratamiento de cada una de las posibles acciones se encuentra al final del
fichero:
\begin{itemize}
\item \texttt{Inserir: } En este caso, hace unas sustituciones de caracteres
  para que la entrada tenga el formato adecuado para los
  diccionarios, e insiere las cadenas \texttt{\$cad\_LR, \$cad\_bil,
    \$cad\_RL} en los diccionarios monolingüe origen, bilingüe y monolingüe
  meta respectivamente (\texttt{\$fich\_LR, \$fich\_LRRL,
    \$fich\_RL}). Si se produce algún tipo de error al inserir
  la entrada, informa de este hecho con una notificación por pantalla. Si
  \textit{insertarPro.php} había sido llamado desde un procedimiento de validación
  de palabras (\textit{\$valida=1}), entonces insiere un
  botón "\textit{Continue}" para seguir validando palabras. En
  caso contrario insiere un botón de cerrar la ventana que nos permitirá
  acabar.
\item  \texttt{Eliminar: } En este caso, elimina la entrada del fichero de entradas pendientes de validar.
\end{itemize}

\subsubsection{ins\_multip.php}

Realiza las mismas operaciones que \textit{insertarPro.php} pero
cuando la entrada a inserir es una unidad multipalabra. La diferencia
principal es que ahora disponemos de dos variables adicionales
\texttt{\$LRcua} y \texttt{\$RLcua} que contienen la parte invariable de la
multipalabra. Cuando se insiere la entrada en los diccionarios se tiene que añadir
esta cola en el lugar conveniente y convertir los espacios en blanco
por la etiqueta de blanco \texttt{<b/>}.

\subsubsection{insertar.php}

La función de este módulo es muy sencilla. Compone una cadena de texto
con la información proveniente de \textit{selec.php} separada por
tabuladores. Esta cadena contiene toda la información necesaria para
generar una entrada de diccionario:
...
con las entradas pendientes de validar inseridas por usuarios no
profesionales. Cuando un usuario considerado profesional quiera validar
entradas pendientes, el módulo \textit{valida.php} leerá este mismo
fichero.


\subsubsection{verSemi.php}
Se encarga de mostrar por pantalla el fichero de entradas pendientes de validación,
y lo hace así: lee el fichero con las entradas
(\textit{\$fichSemi}) y entra en un bucle que recorre todas las
entradas que se encuentran en el fichero. Para cada una de ellas muestra una
...
\subsection{Ficheros de diccionario}

Los ficheros con las entradas inseridas desde el formulario se encuentran
en \texttt{/dics}. Aquí se encuentran dos tipos de ficheros:

\begin{itemize}
\item \texttt{apertium-ll-rr.xx.webform}: El fichero que contiene entradas
  en código \texttt{XML} preparadas para ser copiadas en los
  diccionarios finales. El nombre del fichero tiene el formato mostrado, siendo
  \texttt{ll-rr} las iniciales correspondientes al traductor al que hace
  referencia el fichero y \texttt{xx} las iniciales de la lengua del
  monolingüe al que hace referencia o las iniciales del bilingüe. Por
  ejemplo, las iniciales del traductor español-catalán son
  \texttt{es-ca}. Para este traductor tenemos el monolingüe de español
  (\texttt{es}), el de catalán (\texttt{can}) y el bilingüe
  (\texttt{es-ca}) Así, en este directorio tendremos los siguientes
  ficheros para el traductor español-catalán:
\begin{center}
\texttt{apertium-es-ca.es.webform	apertium-es-ca.ca.webform	apertium-es-ca.es-ca.webform}
\end{center}


\item \texttt{oo-mm.semi.dic}: El fichero que contiene las entradas
  pendientes de ser validadas de un determinado par de lenguas. \texttt{oo-mm}
  son las iniciales correspondientes al par concreto. Por ejemplo, el
  traductor español-catalán tendrá este fichero para el
  semi-profesional: \texttt{es-ca.semi.dic}


\end{itemize}

\subsection{Ficheros de paradigmas}

Los paradigmas utilizados para cada par de lenguas se especifican
en dos archivos de formato \texttt{XML} denominados
\texttt{paradig.ll-rr.xx.xml} donde \texttt{xx} son las iniciales
correspondientes a una lengua y \texttt{ll-rr} las iniciales
correspondientes a un par de lenguas. Estos archivos están formados por
un conjunto de entradas correspondientes a paradigmas o modelos de flexión de las palabras 
de una lengua en concreto. El fichero \texttt{XML} se compone de las siguientes partes:
\begin{itemize}
\item Cabecera/Raíz del documento de especificación.\\
\begin{alltt}
<?xml version="1.0" encoding="ISO-8859-1"?>
...
\end{alltt}
  El atributo \textit{lang} indica las iniciales de la lengua para la cual
  están especificándose paradigmas y el atributo
  \textit{langpair} indica las iniciales correspondientes al par de lenguas
  del traductor al que corresponde el fichero. Es necesario que
  en el mismo directorio donde están los ficheros de paradigmas esté
  \texttt{form.dtd}, donde se especifica la DTD que siguen estos
  ficheros. Esta DTD está detallada en el anexo \ref{ss:dtdparadigmes}. 
\item Un conjunto de elementos que definen los paradigmas. Para
  explicar el formato de los elementos reproducimos este ejemplo:
  \\
\begin{alltt}
...
\end{alltt}
  Cada paradigma está especificado en un elemento \texttt{<entry>}.
  Este elemento puede tener tres atributos:
  \begin{itemize}
  \item \textit{PoS}: la categoría gramatical del paradigma. Puede
    tomar los valores: acr, adj, adv, noun, pname, pr, verbo. \nota{faltaria cnjadv?} Es
    obligatorio para cualquier categoría gramatical.
  \item \textit{nbr}: los números admitidos por el paradigma. Puede tener
    los valores: sg, pl, sg\_pl, sp.
  \item \textit{gen}: los géneros admitidos por el paradigma. Puede tener
    los valores: m, f, m f, mf.
  \end {itemize}
  Además, está formado por dos elementos:
  \begin{itemize}
  \item \texttt{endings}: la raíz y las terminacions utilizadas para
    seleccionar el paradigma en el formulario y mostrar los ejemplos de flexión.
    
  \item \texttt{paradigms}: especificación del paradigma o paradigmas
    que definen la flexión seguida por una entrada en concreto.
    Necesita del atributo \textit{howmany} que indica el número de paradigmas
    que utiliza la entrada. Para cada uno de los
    paradigmas utilizados hay una línea que especifica el nombre del
    paradigma del diccionario, y que tiene el siguiente formato:
    \begin{center}
\begin{alltt}
<par n="long\_\_adj"/>
\end{alltt}           
...
A partir del fichero de paradigmas escrito en \texttt{XML}, se tienen que
generar los ficheros con los que directamente trabajan los módulos de los
formularios. Si se ejecuta el script \texttt{/private/gen\_paradig.sh}
el proceso se realiza automáticamente para todos los pares de lenguas
de que disponemos:
\begin{alltt}
   #  cd private
   #  ./gen\_paradig.sh
\end{alltt}
Para añadir un nuevo paradigma a los formularios, debe
prepararse la entrada correspondiente en el fichero de paradigmas en formato
\texttt{XML} y después actualizar los archivos de trabajo con la orden
anterior.

El proceso automatizado también se puede realizar de forma manual si no
queremos actualizar los ficheros de todos los pares de lenguas que
tenemos instalados. La generación manual de los archivos de trabajo
se hace con una hoja de estilo \texttt{XSL} mediante
esta orden:
\begin{alltt}
   #  xsltproc paradigmas.xsl fichero\_de\_paradigmas.xml 
                                     | ./creaparadig.awk
\end{alltt}

Esta acción genera un fichero de trabajo para cada categoría
gramatical. Los ficheros generados se encuentran en los directorios \texttt{/private/paradigmas.ll-rr}.
Cada directorio contiene ficheros con los paradigmas que se pueden utilizar
para cada par de lenguas \texttt{ll-rr} y para cada componente gramatical.
Cada uno de estos directorio contiene los ficheros siguientes:
\begin{itemize}
\item \texttt{paradigacr\_xx}: paradigmas para acrónimos de la lengua
  \texttt{xx}.
\item \texttt{paradigadj\_xx}: paradigmas para adjetivos de la lengua
  \texttt{xx}.
\item \texttt{paradigadv\_xx}: paradigmas para adverbios de la lengua
  \texttt{xx}.
\item \texttt{paradigcnjadv\_xx}: paradigmas para conjunciones
  adverbiales de la lengua \texttt{xx}.
\item \texttt{paradigcnjcoo\_xx}: paradigmas para conjunciones
  copulativas de la lengua \texttt{xx}.\nota{MG: aquesta no està en la pàgina web del formulari}
\item \texttt{paradigcnjsub\_xx}: paradigmas para conjunciones
  subordinadas de la lengua \texttt{xx}.\nota{ídem}
\item \texttt{paradignoun\_xx}: paradigmas para nombres de la lengua
  \texttt{xx}.
\item \texttt{paradigpname\_xx}: paradigmas para nombres propios de la
  lengua \texttt{xx}.
\item \texttt{paradigpr\_xx}: paradigmas para preposiciones de la
  lengua \texttt{xx}.
\item \texttt{paradigverb\_xx}: paradigmas para verbos de la lengua
  \texttt{xx}.
\end{itemize}

Los ficheros están compuestos por una entrada por línea. Cada entrada
contiene la información siguiente:

\begin{center}
% uso packages: array
\begin{tabular}{lllll}
\textit{ejemplos} & \textit{número de paradigmas} & \textit{paradigmas\_modelo} & \textit{(números)} & \textit{(géneros)}
\end{tabular}
\end{center}


El separador utilizado para las diferentes partes de la entrada es el
tabulador.
\begin{itemize}
\item \textit{Ejemplos}: son las terminacions que se utilizarán para
  generar los ejemplos cuando escogemos este paradigma como prototipo
  para una determinada palabra a inserir.
\item \textit{Número de paradigmas}: es el número de paradigmas del
  diccionario que se utilizarán para flexionar adecuadamente este
  modelo de flexión en particular.
\item \textit{Paradigmas modelo}: es el nombre del paradigma o paradigmas
  que están en el diccionario y que son los que se utilizarán para
  flexionar la nueva entrada.
\item \textit{(Números)}: Sólo se pone en el caso de nombres, adjetivos y acrónimos.
  Hace referencia al número del paradigma.
\item \textit{(Géneros)}: Sólo se pone en el caso de nombres, adjetivos y acrónimos.
  Hace referencia al género del paradigma.
\end{itemize}

Así por ejemplo, en el traductor español-catalán tendríamos el directorio  \texttt{/private\-/paradigmas.es-ca} 
donde encontraríamos dos ficheros \texttt{XML}: \texttt{paradig.es-ca.es.xml}
y \texttt{paradig.es-ca.ca.xml} con la especificación de los paradigmas
utilizados en cada lengua. A partir de estos ficheros se pueden generar todos 
los ficheros de paradigmas para un par de lenguas concreto con la orden:
\begin{alltt}
...
O bien, se pueden generar automáticamente para todos los pares de lenguas con:
\begin{alltt}
   #  ./private/gen\_paradig.sh
\end{alltt}

Entre los ficheros de trabajo generados, habría uno, por ejemplo, denominado \texttt{paradigverb\_ca} que contendría
los posibles paradigmas de verbos para el catalán, donde una posible línea sería la siguiente:

\begin{center}
\texttt{abra/çar /ço /ci        1       abalan/çar\_\_vblex}
\end{center}

que deriva de la entrada en \texttt{XML}:
...
\section{Utilización de los formularios}
\label{ss:formus}
\subsection{Introducción}

\notavisible{Intentar evitar el llenguatge sexista en tot el document:
  \emph{usuari}, etc.}  
Cuando la persona usuaria quiera inserir
nuevas entradas a un diccionario tiene que conectarse mediante un
cliente navegador a la dirección donde se haya instalado el servidor
de formularios, por ejemplo:
\begin{center}
\texttt{http://xixona.dlsi.ua.se/forms}
\end{center}
Se abrirá una página web con el portal de entrada a \texttt{Opentrad\- Apertium\- Insertion\- Form}. En el marco de la izquierda hay 
enlaces para obtener más \textit{información} , \textit{descargar} los programas y \textit{contacto} para solicitar
inscribirse como usuario del sistema. Para inscribirse como usuario hay que enviar un correo electrónico al
\emph{webmaster} encargado de los formularios.

\notavisible{Canviar a tot arreu \emph{registrar} per \emph{inscribir}.}
Para inserir nuevas palabras, se llenará el formulario con los datos correspondientes y se
pulsará el botón \textit{'Go On'}; en este momento habrá que identificarse como usuario
inscrito o no se podrá continuar con el procedimiento de inserir palabras. Cuando
una persona se inscribe se puede registrar de dos maneras: como
\emph{profesional} o como \emph{no profesional}. Cada modalidad tiene unas habilitacions
diferentes, que se explicarán en los apartados siguientes.

\subsection{Inserción de entradas}

\subsubsection{Perfil profesional}

Cuando se quiere inserir una nueva entrada a los diccionarios hay que dirigirse al
apartado correspondiente al par de lenguas que se quiera ampliar. Entonces
se especifica el lema en lengua origen y en lengua meta y se elige la categoría
gramatical de las entradas que se inserirán. A continuación, se pulsa el
botón \textit{Go on} para seguir con el proceso de inserción.

En este punto aparece una nueva ventana que muestra los lemas y unos
parámetros que caracterizan la nueva entrada de diccionario. Si la entrada ya
 está en alguno de los diccionarios, se avisará a la persona usuaria y el sistema
automáticamente escogerá sentido unidireccional (de izquierda a derecha o al
inrevés). Si no está en ninguno, se elegirá doble sentido de traducción. 

En esta
nueva ventana se pueden hacer estas tres acciones:
...
\begin{itemize}
\item Escoger el paradigma para el lema en lengua origen
   y para el lema en lengua meta (esta acción es obligatoria, el resto
  no).\footnote{Escoger el paradigma es un proceso que debe realizarse 
    con mucho cuidado. Hay que elegir el paradigma exacto que describe
    el comportamiento del lema que estamos inseriendo. En
    el caso de adjetivos, nombres y acrónimos, hay que escoger el paradigma que
    se ajuste a la flexión deseada y también a los géneros admitidos. En
    el caso de acrónimos, por ejemplo, hay que fijarse en el género y número que admite
    cada paradigma de los que podemos escoger; por ejemplo BBC sirve
    para acrónimos femeninos singulares y SA para acrónimos femeninos que
    admiten plural). En el caso de nombres propios, debe escogerse el
    paradigma en función de si se trata de un nombre propio de cosa
    (p. ej. un periódico), de persona o de lugar.}
\item Escoger la dirección de traducción de la entrada si es diferente de la
  propuesta automáticamente.
\item Añadir comentarios en la entrada que quedarán en el diccionario final.
\end{itemize}

Una vez se han hecho las operaciones correspondientes, debe pulsarse
\textit{'Go on'} si queremos confirmar la entrada o \textit{'Close'} si
queremos cancelar la operación de inserción.

La siguiente y última pantalla que se nos muestra contiene las tres entradas
en formato \texttt{XML} generadas para los diccionarios monolingüe
lengua origen, bilingüe y monolingüe lengua meta. Estas entradas
se muestran en tres ventanas de texto y se pueden editar si se desea hacer
alguna modificación. Una vez revisadas las entradas, se pulsa el
botón \textit{'Insert'} para inserirlas definitivamente en los
diccionarios correspondientes. También tenemos la posibilidad de pulsar el
botón \textit{'Go back'} si queremos volver al paso anterior.

\subsubsection{Usuario no profesional}

Cuando un usuario entra al sistema de inserción como usuario no
profesional, el procedimiento de inserción de palabras es el mismo que
para el usuario profesional, pero las entradas inseridas no se guardan
en los diccionarios generados por los formularios sino que quedan en una cola
de entradas pendientes de validación. Para inserir las palabras de esta
cola a los diccionarios hace falta que un usuario profesional las valide.

\subsection{Validación de entradas pendientes de aprobación}

Los usuarios profesionales, en la pantalla donde se escoge el paradigma
de los lemas inseridos, tienen dos enlaces adicionales:
\begin{itemize}
\item \textit{See pairs to be validated}: Aquí se abre una ventana donde
  se muestra el contenido del fichero con las palabras que están
  pendientes de validación; son palabras que han sido inseridas por
  usuarios no profesionales. Esta ventana es meramente informativa y se
  cierra pulsando el botón \textit{'Close'}.
\item \textit{Validate pairs}: Esta opción permite a un usuario
  profesional validar una a una las entradas pendientes. Al pulsar este botón
  se abre la ventana de selección de paradigmas que se ha
  visto en el apartado de inserción de nuevas palabras. Esta ventana
  muestra los datos que ha escogido el usuario para la entrada que ha añadido.
  Ahora, el usuario profesional puede modificar los lemas,
  borrar la entrada (botón \textit{'Delete'}) o seguir con el proceso
  de inserción. Si sigue con la inserción de la entrada, el proceso es el
  mismo que en una inserción normal, sólo que cuando la entrada queda
  definitivamente añadida a los diccionarios de formulario el control vuelve
  a mostrar la siguiente entrada a validar de la cola.

Este proceso es repite hasta que se validen todas las palabras de la cola o se finalice el proceso pulsando el botón \textit{'Close'}.
