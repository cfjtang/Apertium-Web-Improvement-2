...
To calculate the naïve coverage\footnote{FIXME} of the analyser, two corpora were used,
 the first was a database dump of the Kyrgyz Wikipedia, dated XX-XX-XX,\footnote{DUMP NAME} 
which was processed with the programs YY and ZZ to extract sentences. The second was FIXME. 

Both corpora were split into 10 equal parts, and coverage was calculated over each part 
separately in order to calculate the standard deviation of the mean. As can be seen 
from table~\ref{table:cov}, running the analyser on the RFERL gives a higher coverage, 
because the text is more homogenous, and there is less non-Kyrgyz (e.g. Russian) text.
...
\hline
Kyrgyz Wikipedia & 329,524   & 269,573   & 81.84 $\pm$ 3.23  & 4.53\\
RFERL Kyrgyzstan & 4,107,555 & 3,586,000 & 87.37 $\pm$ 1.23  & 4.26\\
\hline
\end{tabular}
\caption{FIXME}
\label{table:cov}
...
The column 'mean ambiguity' gives the average number of analyses for each surface form 
given when analysing this corpus.
