	...
	for item in defCats:
		...
		for element in elements:
			...
			if element in reverseMap:
				print("Repeated cat-item",element ,"found for entries :", item, reverseMap[element])
			else:
	...
	for entry in rules:
		for elements in rules[entry]:
			...
			if 'pos' in elements[1]:
				...
				if pos > validMaxPos :
					print(errorsConf["checkValidPosition"]["message"],tag, pos, entry)
			...
			for tags in rule.iter():
				...
				if 'lu' in childList:
					...
					if luCount > bCount + 1:
						...
						element = tags.tag
						print(errorsConf["enforceBreakTag"]["message"] % (element, lineNum))
				...
				if element == 'out':
					for child in tags.iter():
						...
						if 'side' in attributes and attributes['side'] == 'sl':
							print (errorsConf["enforceSide"]["message"] % (child.tag, child.sourceline))
...
def checkValidPartClip():
	...
	for x in validParts: tmpList.append(x)
		...
		for attribs in rules[entry]:
			if 'clip' in attribs:
				...
				if part not in validParts:
					print(errorsConf["checkValidPartClip"]["message"] % (entry, part))
...
def compareClipLit(element):
	...
	for child in element:
		...
		if child.tag == "lit":
			v = child.attrib['v']
		if v == None and child.tag == "lit-tag":
			v = child.attrib['v']
		...
		for rule in section:
			for tags in rule.iter():
				...
				if element == "equal":
					...
					if None in validationTuple : continue
					try:
						if validationTuple[1] not in defAttrs[validationTuple[0]]:
							print (errorsConf["checkValidEqualTag"]["message"], validationTuple, " line number : ", tags.sourceline)
					except KeyError:
...
def main(arg_list):
	...
	#for x in rules["REGLA: ADV ADJ TO INF ENC (BCN: canviem la prep TO per DE; els adjectius que no compleixin la regla s'han de codificar al lèxic. Queda com a GD i ND fins al interchunk)"]:
	#	print (x)
