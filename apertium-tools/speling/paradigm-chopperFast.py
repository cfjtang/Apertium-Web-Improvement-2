...
def roll_right(name):
        ...
        return name[bar_idx:udr_idx]


def compare_paradigms(paradigm1, paradigm2): 
        # paradigm,stem,symbols !! paradigm,stem,symbols        
        paradigm1Len = len(paradigm1) # this is for maximixing speed
        if paradigm1Len != len(paradigm2): 
                return False

        common = 0

        for pair1 in paradigm1: 
                common += paradigm2.count(pair1)

        if common == paradigm1Len: 
                return True
...
#       print 'paradigms: ' , len(paradigms)
        ...
        for paradigm1 in list(paradigms.keys()): 
                if current != paradigm1 and paradigm1 in paradigms and current in paradigms: 
