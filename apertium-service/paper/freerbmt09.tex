...
To evaluate the efficiency of the service we realized, we compared the time it requires to compute and answer to a translation request with the time require by Moses~\citep{moses}, 
a Statistical Machine Translation system. The translation model used by Moses has been trained on the well-known Europarl~\citep{europarl} corpus on the Spanish to English
pair using SRILM~\citep{srilm}, compiled in binary format using IRSTLM~\citep{irstlm} and then trained to minimize the error rate on a set of sentences from the same corpus.

We compared the efficiency of our service with  .. Moses~\citep{moses}.\\

Moses was trained on the \emph{News Commentary} corpus provided on \emph{Fourth Workshop on Statistical Machine Translation}'s website\footnote{\url{http://www.statmt.org/wmt09/}}.\\
