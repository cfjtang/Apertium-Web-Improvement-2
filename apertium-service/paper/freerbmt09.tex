...
\end{center}
\caption{Comparison in the ``Sentence Length - Time'' space between {\tt apertium}, {\tt apertium-service}, {\tt moses} and {\tt moses-service}; measurements are in $log_{2}(string\ length)$ for Sentence Length and in $log_{2}(ms)$ for Time.}
\label{fig:comp}
\end{figure}

%To evaluate the efficiency of our service (which we will refer to as {\tt apertium-service}), we compared the time it requires to compute and answer to a translation request for the following systems:
...
Both {\tt apertium-service} and {\tt moses-service} were accepting translation requests in XML-RPC format, and the free text used for timing all the systems was also taken from Europarl corpus. Results are shown in Figure ~\ref{fig:comp}.
