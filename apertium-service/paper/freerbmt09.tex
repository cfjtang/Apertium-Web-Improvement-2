...
In all methods, languages are represented by their ISO 639-1~\citep{ISO:639-1} code. Figure \ref{fig:pythoninv} shows a short example on how our service's XML-RPC interface can be invoked by the Python\footnote{\url{http://www.python.org/}} shell.
...
>>> print proxy.translate("Test for the machine 
	...
	["translation"]
Prueba para el servicio de traducción de la 
	máquina
 \end{Verbatim}
...
\begin{description}
 ...
 \item[Predictability] -- because direct acquisition of a resource from an external resource enviroment (for example, a filesystem or a DBMS) can be characterised by nondeterministic timing~\citep{realtime};
 \item[Stability] -- Because repetitious acquisition and release of resource can increase the risk of system instability because of, for example, memory fragmentation problems typical of heap-based allocation~\citep{robust, realtime}); %% SHOULD YOU PUT A REF HERE ? -FMT
