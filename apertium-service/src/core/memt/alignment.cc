...
Alignment::Alignment(std::vector<wstring>& input_lines) {
	...
	for (std::vector<wstring>::iterator it = input_lines.begin(); it
			!= input_lines.end(); ++it) {
...
void Alignment::match(Matcher& m) {
	for (std::vector<Pairwise_Alignment>::iterator it = _pw_alignments.begin(); it
			!= _pw_alignments.end(); ++it)
		it->match(m);
}
...
void Alignment::align(Aligner& aligner, unsigned int ngram_level) {
	...
	for (std::vector<Pairwise_Alignment>::iterator it = _pw_alignments.begin(); it
		...
		for (unsigned int iright = 0; iright < it->_final_alignment.size(); ++iright) {
			if (it->_final_alignment[iright] != -1) {
				_aligned[it->_index_mte_right][iright].push_back(std::pair<
						unsigned int, int>(it->_index_mte_left,
						it->_final_alignment[iright]));
			}
		}
		for (unsigned int ileft = 0; ileft < it->_final_alignment_left.size(); ++ileft) {
			if (it->_final_alignment_left[ileft] != -1) {
				_aligned[it->_index_mte_left][ileft].push_back(std::pair<
						unsigned int, int>(it->_index_mte_right,
	...
	unsigned int transitivity_depth = TRANSITIVITY_DEPTH
			< (_mt_translations.size() - 1) ? TRANSITIVITY_DEPTH
			: (_mt_translations.size() - 2);
	for (unsigned int i = 1; i < transitivity_depth; ++i) {
		for (unsigned int mt = 0; mt < _aligned.size(); ++mt) {
			for (int nword = 0; nword < static_cast<int> (_aligned[mt].size()); ++nword) {
				std::list<std::pair<unsigned int, int> > temp_list(
						_aligned[mt][nword]);
				for (std::list<std::pair<unsigned int, int> >::iterator it1 =
						_aligned[mt][nword].begin(); it1
						!= _aligned[mt][nword].end(); ++it1) {
					for (std::list<std::pair<unsigned int, int> >::iterator
							it2 = _aligned[it1->first][it1->second].begin(); it2
							!= _aligned[it1->first][it1->second].end(); ++it2) {
						if (it2->first != mt || it2->second != nword) {
							temp_list.push_back(std::pair<unsigned int, int>(
	...
	for (unsigned int mt = 0; mt < _aligned.size(); ++mt) {
		for (int nword = 0; nword < static_cast<int> (_aligned[mt].size()); ++nword) {
			...
			for (list<pair<unsigned int, int> >::iterator al =
					_aligned[mt][nword].begin(); al
					!= _aligned[mt][nword].end(); ++al) {
				// we check for the followings
				for (unsigned int ngram = 1; ngram < ngram_level; ++ngram) {
					if (al->second
							< static_cast<int> (_aligned[al->first].size()) - 1
							&& nword + ngram < _aligned[mt].size()) {
						int ind_word = nword + ngram;
						pair<unsigned int, int> to_find = pair<unsigned int,
								int> (al->first, al->second + ngram);
						list<pair<unsigned int, int> >::iterator f = find(
								_aligned[mt][ind_word].begin(),
								_aligned[mt][ind_word].end(), to_find);
						if (f != _aligned[mt][ind_word].end()) {
							_ngrams_aligned[ngram - 1][mt][nword].push_back(
									pair<unsigned int, int> (al->first,
											al->second));
						}
					}
				}
...
void Alignment::print_aligned() {
	...
	for (std::vector<std::vector<std::list<std::pair<unsigned int, int> > > >::iterator
			it1 = _aligned.begin(); it1 != _aligned.end(); ++it1) {
		for (std::vector<std::list<std::pair<unsigned int, int> > >::iterator
				it2 = it1->begin(); it2 != it1->end(); ++it2) {
			wcout << "{ ";
			for (std::list<std::pair<unsigned int, int> >::iterator c =
					it2->begin(); c != it2->end(); ++c) {
		...
		for (unsigned int mt = 0; mt < _aligned.size(); ++mt) {
			for (int nword = 0; nword < static_cast<int> (_aligned[mt].size()); ++nword) {
				...
				for (list<pair<unsigned int, int> >::iterator it =
						_ngrams_aligned[ngram][mt][nword].begin(); it
						!= _ngrams_aligned[ngram][mt][nword].end(); ++it) {
...
void Alignment::print_mt_translations() {
	...
	for (std::vector<std::vector<wstring> >::iterator it1 =
			_mt_translations.begin(); it1 != _mt_translations.end(); ++it1) {
		for (std::vector<wstring>::iterator it2 = it1->begin(); it2
				!= it1->end(); ++it2) {
			wcout << *it2 << " | ";
		}
		wcout << endl;
